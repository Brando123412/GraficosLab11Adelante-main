Shader "Custom/ShaderSemana12"
{
    Properties{
        ...
        _Ilumination("Ilumination", 2D) = "white" {}
        _ColorIluminacion("ColorIluminacion", Color) = (1,1,1,1)
        _ScalarIlumination("ScalarIlumination", Range(0,10)) = 1
        ...
        uniform float4 _ColorIluminacion;

        uniform sampler _MainTex;
        uniform sampler _Ilumination;
        uniform sampler _SpecularMap;
        uniform sampler _NormalMap;
        uniform sampler _AO;
        
        uniform float _ScalarIlumination;
        ...
        float4 fs_main(VS_OUT input) : COLOR
        {
            ...
            float3 Iluminacion = tex2D(_Ilumination, input.uv).rgb;
            ...
            float4 finalCol = col;
            if (Iluminacion.r>0 && Iluminacion.g > 0 && Iluminacion.b > 0) {
                finalCol.rgb*= Iluminacion.rgb * _ColorIluminacion.rgb * _ScalarIlumination;
            }
